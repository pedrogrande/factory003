  ...
  def create
    ...
    respond_to do |format|
      if @scholarship.save
      	...
      	ScholarshipMailer.scholarship_received(@scholarship).deliver
      	logger.info {"Starting email second"}
      	ScholarshipMailer.scholarship_response(@scholarship).deliver

        format.html { redirect_to thanks_path, notice: 'Scholarship was successfully created.' }
